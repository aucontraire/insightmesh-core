...
- [ ] T001 Confirm branch `005-page-provenance`; run `uv sync --all-extras`; verify `pyproject.toml` requires no new dependencies for this feature (Pydantic v2, PyYAML, echomine, Typer, claude-agent-sdk all already present; `subprocess` for `git` invocation is stdlib per Research Decision R1).
