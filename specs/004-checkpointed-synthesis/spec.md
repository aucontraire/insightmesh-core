...
- **FR-009**: When the user provides a positive per-invocation cap of N, System MUST process at most N additional exchanges this invocation. The cap constrains BOTH dimensions: (a) each checkpoint's slice contains at most the remaining-cap exchanges (slice size capped at min(token_budget, remaining_cap)), and (b) after each successful checkpoint, if the count of exchanges processed has reached or exceeded N, processing stops. The cursor's resting point is always a checkpoint boundary. Note: the original spec framing of "soft cap evaluated only between checkpoints" was insufficient — it meant small conversations fitting in one token-budget-sized checkpoint would ignore the cap entirely. Per-slice constraint is required for the cap to fire on small transcripts.
...
- **SC-006**: When a per-invocation cap of N exchanges is passed (and the transcript has at least N remaining unprocessed exchanges), the cursor's `last_processed_exchange_index` advances by exactly N exchanges — the cap is enforced both at slice-size determination and between checkpoints. The cursor's resting point is always a checkpoint boundary. The cap is measured in exchanges, not tokens or characters.
